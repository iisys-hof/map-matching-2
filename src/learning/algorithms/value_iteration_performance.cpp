...
#include "value_iteration_performance.hpp"
...
namespace map_matching_2::learning {
    ...
    value_iteration_performance<Environment>::value_iteration_performance(
            Environment &environment, const learning::settings &settings)
            : _environment{environment}, _settings{settings} {}

    template<typename Environment>
    std::size_t value_iteration_performance<Environment>::generate_states(std::vector<state_type> &states) {
...
//            std::cout << "Final States: " << states.size() << std::endl;
    ...
    template<typename Environment>
    std::vector<std::pair<std::size_t, std::size_t>> value_iteration_performance<Environment>::operator()() {
...
//                std::cout << "Episode: " << episode << ", value: " << std::setprecision(20) << value << std::endl;
    ...
    template
    class value_iteration_performance<environment::single_performance<matching::types_geographic::matcher_static>>;

    template
    class value_iteration_performance<environment::single_performance<matching::types_cartesian::matcher_static>>;
