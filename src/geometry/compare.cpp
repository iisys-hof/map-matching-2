        ...
        if (error_fraction == default_float_type<length_type>::v0) {
            ...
            if (ground_truth.has_length() and ground_truth.length() > default_float_type<length_type>::v0 and
                ...
                correct_fraction = correct / std::max(ground_truth.length(), compare.length());
                    ...
                    (not ground_truth.has_length() or ground_truth.length() == default_float_type<length_type>::v0) and
    ...
    void line_comparator<RichLine>::_adapt_points(std::list<rich_segment_type> &segments,
        ...
        auto adapt_point = [&](auto &segment_it, const bool first) {
            ...
            if (nearest_point != nullptr) {
                *segment_it = first ? rich_segment_type{*nearest_point, segment.second}
                                    : rich_segment_type{segment.first, *nearest_point};
                if (not(segment_it == segments.begin() and
                    ...
                    if (not geometry::equals_points(next_segment.segment().first, point)) {
                        next_segment = rich_segment_type{point, next_segment.segment().second};
                    }
                }
            }
        };
