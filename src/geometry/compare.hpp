    ...
    public:
        ...
        compare(const multi_rich_line_type &ground_truth, const multi_rich_line_type &compare) const {
            ...
            if ((a_segments_list.empty() and b_segments_list.empty())) {
                return comparison<rich_segment_type>{default_float_type<length_type>::v1,
                ...
                return comparison<rich_segment_type>{default_float_type<length_type>::v0,
                                                     std::numeric_limits<length_type>::infinity(),
                                                     default_float_type<length_type>::v0,
                                                     compare.has_length() ? compare.length()
                                                                          : default_float_type<length_type>::v0,
                                                     default_float_type<length_type>::v0,
            ...
            if (error_fraction == default_float_type<length_type>::v0) {
                ...
                if (ground_truth.has_length() and ground_truth.length() > default_float_type<length_type>::v0 and
                    ...
                    correct_fraction = correct / std::max(ground_truth.length(), compare.length());
                        ...
                        (not ground_truth.has_length() or
                         ground_truth.length() == default_float_type<length_type>::v0) and
            ...
            return comparison<rich_segment_type>{correct_fraction, error_fraction, correct, error_added, error_missed,
                                                 std::move(corrects), std::move(error_adds), std::move(error_misses)};
        }

        comparison<rich_segment_type>
        compare(const rich_line_type &ground_truth, const rich_line_type &compare) const {
        ...
        _adapt_points(std::list<rich_segment_type> &segments, const std::list<rich_segment_type> &adopt_segments,
            ...
            auto adapt_point = [&](auto &segment_it, const bool first) {
                ...
                if (nearest_point != nullptr) {
                    *segment_it = first ? rich_segment_type{segment_type{*nearest_point, segment.second}}
