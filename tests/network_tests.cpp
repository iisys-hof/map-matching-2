...
BOOST_AUTO_TEST_SUITE(network_tests)
    ...
    BOOST_FIXTURE_TEST_CASE(edge_test, network_fixture<network_import_metric>) {
        const auto line = create_line<point_type>({0, 0, 0, 1, 0, 2, 0, 3, 0, 4});
    ...
    BOOST_FIXTURE_TEST_CASE(network_creation_test, network_fixture<network_import_metric>) {
        ...
        const auto &edge_d = add_edge(network, 4, {{4, 5},
                                                   {5, 6},
                                                   {6, 7}});

        BOOST_CHECK_EQUAL(boost::num_vertices(network.graph()), 7);
        ...
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_b).at(0).get<0>(), 0, 1e-6);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_b).at(0).get<1>(), 1, 1e-6);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_b).length(), 1.0, 1e-6);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_b).azimuth(), 90, 1e-6);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_c).at(1).get<0>(), 1, 1e-6);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_c).at(1).get<1>(), 2, 1e-6);
        BOOST_CHECK_EQUAL(network.edge(edge_d).id, 4);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_d).length(), 3.0, 1e-6);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_d).azimuth(), 0, 1e-6);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_d).directions(), 180, 1e-6);
    }
    ...
    BOOST_FIXTURE_TEST_CASE(network_conversion_test, network_fixture<network_import_metric>) {
        ...
        BOOST_CHECK_EQUAL(boost::num_vertices(network_import.graph()), 3);
        BOOST_CHECK_EQUAL(boost::num_edges(network_import.graph()), 3);

        network_metric network;
        network_import.convert(network);
    ...
    BOOST_FIXTURE_TEST_CASE(network_reprojection_test, network_fixture<network_import_geographic>) {
        ...
        BOOST_CHECK_EQUAL(boost::num_vertices(network.graph()), 4);
        BOOST_CHECK_EQUAL(network.vertex(vertex_a).id, 1);
        ...
        const auto edge_a = add_edge(network, 1, {{1, 3},
                                                  {3, 2}});
        const auto edge_b = add_edge(network, 2, {{2, 4},
                                                  {4, 1}});
        const auto edge_c = add_edge(network, 3, {1, 2});

        BOOST_CHECK_EQUAL(boost::num_edges(network.graph()), 3);
        BOOST_CHECK_EQUAL(network.edge(edge_a).id, 1);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_a).length(), 21516.585, 1e-2);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_a).azimuth(), 50.274, 1e-2);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_a).directions(), -89.936, 1e-2);
        BOOST_CHECK_EQUAL(network.edge(edge_b).id, 2);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_b).length(), 21516.585, 1e-2);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_b).azimuth(), -129.599, 1e-2);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_b).directions(), -90.064, 1e-2);
        BOOST_CHECK_EQUAL(network.edge(edge_c).id, 3);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_c).length(), 15288.437, 1e-2);
        BOOST_CHECK_CLOSE_FRACTION(network.edge(edge_c).azimuth(), 50.274, 1e-2);
        BOOST_CHECK(not network.edge(edge_c).has_directions());
        ...
        BOOST_CHECK_EQUAL(boost::num_vertices(network_reprojected.graph()), 4);
        ...
        BOOST_CHECK_CLOSE_FRACTION(network_reprojected.edge(edge_a_r).length(), 33684.396, 1e-2);
        BOOST_CHECK_CLOSE_FRACTION(network_reprojected.edge(edge_a_r).azimuth(), 50.260, 1e-2);
        BOOST_CHECK_CLOSE_FRACTION(network_reprojected.edge(edge_a_r).directions(), -90.0, 1e-2);
        BOOST_CHECK_EQUAL(network_reprojected.edge(edge_b_r).id, 2);
        BOOST_CHECK_CLOSE_FRACTION(network_reprojected.edge(edge_b_r).length(), 33684.396, 1e-2);
        BOOST_CHECK_CLOSE_FRACTION(network_reprojected.edge(edge_b_r).azimuth(), -129.740, 1e-2);
        BOOST_CHECK_CLOSE_FRACTION(network_reprojected.edge(edge_b_r).directions(), -90.0, 1e-2);
        BOOST_CHECK_EQUAL(network_reprojected.edge(edge_c_r).id, 3);
        BOOST_CHECK_CLOSE_FRACTION(network_reprojected.edge(edge_c_r).length(), 23919.186, 1e-2);
        BOOST_CHECK_CLOSE_FRACTION(network_reprojected.edge(edge_c_r).azimuth(), 50.260, 1e-2);
        BOOST_CHECK(not network_reprojected.edge(edge_c_r).has_directions());
    }
    ...
    BOOST_FIXTURE_TEST_CASE(network_shortest_path_test, network_fixture<network_import_metric>) {
        ...
        add_edge(network_import, 1, {1, 2});
        add_edge(network_import, 1, {{2, 6},
                                     {6, 4}});
        add_edge(network_import, 1, {4, 5});

        add_edge(network_import, 2, {{1, 7},
                                     {7, 8},
                                     {8, 5}});

        add_edge(network_import, 3, {{1, 9},
                                     {9, 3}});
        add_edge(network_import, 3, {{3,  10},
                                     {10, 5}});
    ...
    BOOST_FIXTURE_TEST_CASE(network_spatial_index_test, network_fixture<network_import_metric>) {
        ...
        BOOST_CHECK_EQUAL(boost::num_vertices(network_import.graph()), 10 * 10);
        BOOST_CHECK_EQUAL(boost::num_edges(network_import.graph()), 4 * 9 * 10);
    ...
    BOOST_FIXTURE_TEST_CASE(network_storage_test, network_fixture<network_import_metric>) {
        network_import_metric network = generate_simplification_network();
